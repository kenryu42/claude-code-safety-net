...
import * as next from '@/core/io/safe-read';
import {
  describeOutcome,
  snapshotTree,
  type TreeSpec,
...
/**
 * The implementation runs against its own copy of one fixture tree; every call is recorded on
 * what it returned or threw and on what it left on disk.
 */
const SIDES = [['next', next]] as const;
...
/** Snapshots a side with its base stripped, so a recorded tree carries no machine path. */
    ...
    for (const replacement of ['file', 'symlink'] as const) {
      ...
      const spy = spyOn(fs, 'readFileSync').mockImplementation(((
        ...
        if (typeof path === 'number') {
          const policy = join(sideBase('next'), 'root', 'policy.json');
          rmSync(policy);
...
describe('atomic policy writes', () => {
  ...
  test('create missing parents and hand the renamed path to the callback', () => {
    ...
    recordPorted(sideSnapshot('next'), rootFolds(base));
  });
