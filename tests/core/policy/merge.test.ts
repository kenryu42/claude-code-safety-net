...
import { mergeProjectPolicy as mergeWithNext } from '@/core/policy/merge';
...
describe('project policy merge', () => {
  test('the hand-picked pairs agree on the merged policy and the weakening lines', () => {
    for (const pair of FIXED_PAIRS) {
      expect(mergeWithNext(pair.user, pair.project)).toMatchSnapshot();
    }
  });

  test('the hand-picked pairs raise every weakening line the merge can report', () => {
    const lines = FIXED_PAIRS.flatMap((pair) => mergeWithNext(pair.user, pair.project).weakenings);
  ...
  test('a seeded sample of user and project pairs agrees', () => {
    ...
    })).map((pair, row) => [`${row}`, mergeWithNext(pair.user, pair.project)] as const);
