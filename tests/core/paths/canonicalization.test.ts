 ...
 * `/srv/home/tester`, and on a runner it is a different directory again. It runs like every
 * other target and is left out of the record.
 */
const ABOVE_PARENT = '../..';
 ...
 * directory would rewrite the literal `/tmp` rows instead. Such a target runs, unrecorded.
 */
...
describe('supported path variable expansion', () => {
  test('agrees with the shipped expander over the tricky table and the corpus', () => {
    ...
    for (const env of variables) {
      const environment = pairedEnvironments(env, home);
      for (const target of targets()) {
        const thrown = expectSameOutcome(
          () => expandSupportedPathEnvironmentVariables(target, environment),
          pathFolds(),
        );
        if (thrown !== undefined) {
  ...
  test('fails closed on nesting past the cap exactly where the shipped expander does', () => {
    const environment = pairedEnvironments({ TMPDIR: '/tmp' }, home);
    for (const depth of [63, 64, 65, 66]) {
      const nested = `${'${HOME:-'.repeat(depth)}x${'}'.repeat(depth)}`;
      const thrown = expectSameOutcome(
        () => expandSupportedPathEnvironmentVariables(nested, environment),
        pathFolds(),
      );
...
describe('protected path candidates', () => {
  test('canonicalize like the shipped scanner from existing and missing cwds', () => {
    const environment = pairedEnvironments({ HOME: home, TMPDIR: '/tmp' }, home);
    for (const cwd of [root, join(root, 'missing-cwd'), join(root, 'alias')]) {
      for (const target of targets()) {
        const thrown = expectSameOutcome(
          () => normalizeProtectedPathCandidate(target, cwd, environment, createBudget()),
          pathFolds(),
  ...
  test('detect Windows namespaces and MSYS drives like the shipped helpers', () => {
    for (const target of targets()) {
      for (const platform of PLATFORMS) {
        expect(isUnsupportedWindowsNamespacePath(target, platform)).toMatchSnapshot();
        recordPorted(normalizeMsysDrivePath(target, platform), pathFolds());
      }
    }
