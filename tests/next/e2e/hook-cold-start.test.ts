...
import { PORTED_LAYOUT } from '../../../scripts/build-layout';
...
/**
 * The two halves of the Phase 11 hook budget, measured over the ported runtime bundle a published
 * install actually ships. The static half is deterministic: the bytes Node has to read and parse
...
beforeAll(async () => {
  ...
  expect((await buildRuntimeBundles(outdir, PORTED_LAYOUT)).success).toBeTrue();
  // Building the ported layout imports next/gui/assets in-process, which leaves bun's listings of
